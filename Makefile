...
gen-frontend-home:
	@cd app/frontend &&
cwgo server --type HTTP --idl ../../idl/frontend/product_page.proto --service frontend -module github.com/trashwbin/gomall-demo/app/frontend -I ../../idl
...
#使用cwgo生成order服务的客户端代码, 生成的代码在rpc_gen/order目录下,所有的客户端代码都在rpc_gen目录下
.PHONY: gen-order-client
gen-order-client:
	@cd rpc_gen && cwgo client  --type RPC --service order --module github.com/trashwbin/gomall-demo/rpc_gen --I ../idl --idl ../idl/order.proto

#服务端代码生成在app/order目录下，客户端代码生成在rpc_gen/order目录下，我们指定服务端引用的一些实现的接口，这里是kitex_gen
...
gen-order-server:
	@cd app/order && cwgo server --type RPC --service order --module github.com/trashwbin/gomall-demo/app/order --pass "-use github.com/trashwbin/gomall-demo/rpc_gen/kitex_gen" -I ../../idl --idl ../../idl/order.proto
